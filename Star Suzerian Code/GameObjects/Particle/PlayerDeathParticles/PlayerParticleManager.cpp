...
//
//static Quaternion rotation(0.0f, 0.0f, 1.0f, (2.0f * PI / PLAYER_EXPLOSION_PARTICLES_PER_WAVE)); // ( (2.0f * PI) / (float)PLAYER_EXPLOSION_PARTICLES_PER_WAVE));
...
//            drawExplosionParticles(explosionBufferOffset,
//                                   PLAYER_EXPLOSION_PARTICLES_PER_WAVE *
//                                   sizeof(VELOCITY_MULTIPLES) *
//                                   (int) (EXPLOSION_PARTICLE_FRAMES_CUTOFF / FRAMES_BETWEEN_PLAYER_EXPLOSION_WAVES));
//            //(triangleBufferOffset) / 3);
//            if (PRINT_PLAYER_PARTICLES_DRAW_COUNTS) {
...
//
//
//    generateExplosionParticles(player->position);
//
//
...
//
//
//void PlayerParticleManager::generateExplosionParticles(const aiVector3D& spawnPoint) {
//
//    aiVector3D velocity(0.0f, INITIAL_PARTICLE_VELOCITY * (0.01f / TIME_TICK_RATE), 0.0f);
//
//    static bool modify = false;
//
//    std::vector<float> velocityMultiples;
//    velocityMultiples.reserve(sizeof(VELOCITY_MULTIPLES));
//    for (float f : VELOCITY_MULTIPLES) {
//        velocityMultiples.push_back(f);
//    }
//
//    for (int i = 0; i < PLAYER_EXPLOSION_PARTICLES_PER_WAVE; i++) {
//        velocity = rotation.computeRotation(velocity); //Rotate velocity
//        std::cout << "\nVelocity it now: x = " << velocity.x << "   y = " << velocity.y;
//
//        std::vector<float>::iterator iter = velocityMultiples.begin();
//        if (modify) {
//            for ( ; iter != velocityMultiples.end(); iter++) {
//                (*iter) *= VELOCITY_INCREASE;
//            }
//            iter = velocityMultiples.begin(); //Reset iter to start of vector
//        }
//        modify = !modify; //Toggle modify each loop iteration
//
//        for( ; iter != velocityMultiples.end(); iter++) {
//            spawnExplosionParticle(spawnPoint, velocity, (*iter));
//        }
//    }
//}
//
//void PlayerParticleManager::spawnExplosionParticle(const aiVector3D& center,
//                                                   const aiVector3D& velocity,
//                                                   float magnitude) {
//    ///Deletion of the explosion player particles is handled in this class's
//    ///private member function   destroyParticlesFlaggedForDestruction()
//    PlayerParticleExplosion * explosionParticlePtr = new PlayerParticleExplosion(center,
//                                                                                 magnitude * velocity.x,
//                                                                                 magnitude * velocity.y,
//                                                                                 velocity.z,
//                                                                                 75.0f);
//    playerParticles.push_back(explosionParticlePtr);
//}
//
//
//
...
//    glUniform1f(shaderDataExplosion.ulocZoom, GAME_SCALE);//particleToGetUniformsFrom->zoom);
...
//Older version from github that actually works:
//
//  PlayerParticleManager.cpp
//
//  Created by Forrest Miller on 5/29/18.
