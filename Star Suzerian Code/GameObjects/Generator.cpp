...
//        instances[newInstanceIndex]->colBox->
        ...
        //Method 2 (Works, but random_device might not be available on every platform)
        std::uniform_real_distribution<float> distribution(0.0f, 2.0f * ((float)PI) );
        ...
        if (PRINT_WEAPON_SPAWN_ANGLES) {
            std::cout << "thetaZ is " << instances[newInstanceIndex]->thetaZ <<
            " and thetaX is " << instances[newInstanceIndex]->thetaX << " and thetaY is  " << instances[newInstanceIndex]->thetaY << std::endl;
        }
