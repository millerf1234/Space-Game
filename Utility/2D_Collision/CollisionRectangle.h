...
//This class is intended to be set from a model that is oriented in the way that will maximize
//x, y and z values. In other words, rotating should ideally not cause x, y, or z to grow larger
//than the largest of x, y, z when the value are calculated from the model.

//----------------------------------------------------------------------------
//                          !!!IMPORTANT!!!
//  USAGE INSTRUCTIONS: 1) Construct the rectangle with no parameters or with
//                     an array of floats representing x,y,z coordinates or with
//                     an array of 3D vectors representing the points. If
//                     using floats, array size must be a multiple of 3! If
//                     constructing with option 2 or 3, skip step 2.
//
//                      2) If used parameterless construcor, call function
...
public:
    ...
    CollisionRectangle(const CollisionRectangle &) = delete; //No copy constructor for now...
    
    //Destructor
    ~CollisionRectangle();
    
    //No copy assignment operator for now...
    ...
    void translate(const aiVector2D & transl) {this->midpoint += transl;}
    void setMidpointTo(const aiVector2D & newMidpoint) {this->midpoint = newMidpoint;}
     ...
     */
    float closestDistanceTo(const CollisionRectangle& otherRect) const;
